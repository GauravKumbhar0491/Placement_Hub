...
class DB:
    ...
    def insert_info(self, data):
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(f"insert into personal_info  (Full_name ,Date_of_birth,Gender,Email, Contact_no) values (\'{data['Fullname']}\', \'{data['DOB']}\',\'{data['gender']}\',\'{data['email']}\',\'{data['phno']}\')")
            ...
            cursor.execute(f"""insert into other (email,PRN,ERP,ABCid ,When_you_can_join) values (\'{data['email']}\',\'{data['prn']}\',\'{data['erp']}\',\'{data['abc']}\',\'{data['join']}\')
            """)
